#!/usr/bin/env python3
"""Generate latent-space offspring for any V4B generation."""
...
def stable_id(sequence: str, generation: int) -> str:
    ...
    return f"V4B_G{generation:02d}_{digest}"


def collect_historical_sequences(results_root: Path) -> set[str]:
    sequences: set[str] = set()
    patterns = [
        "generation_00/generation_00_candidates.csv",
        "generation_*/generation_*_candidates_pre_apex.csv",
        "generation_*/generation_*_survivors.csv",
    ]
    ...
    for pattern in patterns:
        for path in results_root.glob(pattern):
            ...
            seen_paths.add(path)
            try:
                frame = pd.read_csv(path, usecols=["sequence"], low_memory=False)
            ...
            sequences.update(cleaned.tolist())
    return sequences
...
def main() -> None:
    ...
    parser.add_argument("--results-root", default="v4b/results")
    ...
    if args.n_offspring <= 0:
        raise ValueError("--n-offspring must be positive.")
    if args.proposal_multiplier < 1:
        raise ValueError("--proposal-multiplier must be at least 1.")
    ...
    candidates_path = outdir / f"{prefix}_candidates_pre_apex.csv"
    proposals_path = outdir / f"{prefix}_latent_proposals.npz"
    summary_path = outdir / f"{prefix}_generation_summary.json"

    required_paths = [parents_path, source_path, latent_path, checkpoint_path]
    for path in required_paths:
        if not path.exists():
            raise FileNotFoundError(f"Required file not found: {path}")
    if any(path.exists() for path in (candidates_path, proposals_path, summary_path)) and not args.overwrite:
        raise FileExistsError(f"Generation {args.generation} offspring outputs already exist.")

    parents = pd.read_csv(parents_path, low_memory=False)
    source = pd.read_csv(source_path, usecols=["candidate_id", "sequence"], low_memory=False)
    ...
    required_columns = {"candidate_id", "sequence", "parent_selection_stratum"}
    missing = sorted(required_columns - set(parents.columns))
    if missing:
        raise ValueError(f"Parent table is missing columns: {missing}")

    parent_rows: list[int] = []
    for candidate_id in parents["candidate_id"].astype(str):
        if candidate_id not in id_to_row:
            raise ValueError(f"Parent ID absent from latent archive: {candidate_id}")
    ...
    target_proposals = max(
        args.n_offspring,
        int(np.ceil(args.n_offspring * args.proposal_multiplier)),
    )
    ...
    existing_sequences = collect_historical_sequences(results_root)
    existing_sequences.update(
        source["sequence"].astype(str).str.upper().str.replace(r"\s+", "", regex=True)
    )
    accepted: list[dict] = []
    ...
    with torch.inference_mode():
        for start in range(0, target_proposals, args.batch_size):
            ...
            predicted_lengths = predicted_lengths.clamp(
                args.min_length,
                min(args.max_length, config.max_len),
            )
            ...
            for local, (ids, length) in enumerate(zip(sampled_np, lengths_np)):
                ...
                sequence = module.clean_sequence(module.decode_ids(ids[: int(length)]))
                if not module.is_valid_sequence(
                    sequence,
                ...
                accepted.append(
                    {
                        "candidate_id": stable_id(sequence, args.generation),
    ...
    if not accepted:
        raise RuntimeError(f"No valid Generation {args.generation} offspring were produced.")
    ...
    np.savez_compressed(
        ...
        z=z_output,
        generation=np.full(len(offspring), args.generation, dtype=np.int16),
    )
    ...
    summary = {
        ...
        "historical_sequences_excluded": int(len(existing_sequences)),
        "seed": int(args.seed),
        "parameters": {
    ...
    if len(offspring) < args.n_offspring:
        print(
            f"[V4B] Warning: accepted {len(offspring):,} of {args.n_offspring:,} requested. "
