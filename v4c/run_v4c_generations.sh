...
# Frozen V4B bottlenecks; only offspring scale changes.
N_PARENTS="${N_PARENTS:-512}"
N_OFFSPRING="${N_OFFSPRING:-100000}"
...
if (( START_GENERATION < 1 )); then
  echo "[V4C] START_GENERATION must be at least 1." >&2
  exit 2
fi
if (( END_GENERATION < START_GENERATION )); then
  echo "[V4C] END_GENERATION must be >= START_GENERATION." >&2
  exit 2
fi
if (( END_GENERATION > 10 )); then
  echo "[V4C] END_GENERATION cannot exceed 10 for the frozen comparison." >&2
  exit 2
fi
...
if [[ ! -f "$CHECKPOINT" ]]; then
  echo "[V4C] Checkpoint not found: $CHECKPOINT" >&2
  exit 2
fi
...
REQUESTED_GENERATIONS=$((END_GENERATION - START_GENERATION + 1))
REQUESTED_TOTAL=$((REQUESTED_GENERATIONS * N_OFFSPRING))

cat <<EOF
============================================================
AMP-JEPA-Hybrid V4C million-peptide scale-up
Generations:             $START_GENERATION -> $END_GENERATION
...
for (( GEN=START_GENERATION; GEN<=END_GENERATION; GEN++ )); do
  ...
  if [[ -f "$COMPLETE_MARKER" && -s "$SURVIVORS" && -s "$LATENT_VECTORS" ]]; then
    echo "[V4C:G${GEN_PAD}] Complete marker found; skipping generation."
    continue
  fi

  for required in "$SOURCE_POPULATION" "$SOURCE_METADATA" "$SOURCE_LATENTS"; do
    if [[ ! -s "$required" ]]; then
      echo "[V4C:G${GEN_PAD}] Missing required source file: $required" >&2
      echo "[V4C:G${GEN_PAD}] Initialize Generation 0 with v4c/00_initialize_from_v4b.py." >&2
      exit 3
    fi
  done

  GEN_SEED=$((BASE_SEED + GEN * 1009))
  LATENT_SIGMA=$(schedule_value "$LATENT_SIGMA_START" "$LATENT_SIGMA_END" "$GEN" 10)
  TEMPERATURE=$(schedule_value "$TEMPERATURE_START" "$TEMPERATURE_END" "$GEN" 10)

  echo
  echo "------------------------------------------------------------"
  echo "[V4C:G${GEN_PAD}] Starting"
  echo "[V4C:G${GEN_PAD}] Source population: $SOURCE_POPULATION"
  echo "[V4C:G${GEN_PAD}] Latent sigma:     $LATENT_SIGMA"
  echo "[V4C:G${GEN_PAD}] Temperature:      $TEMPERATURE"
  echo "[V4C:G${GEN_PAD}] Seed:             $GEN_SEED"
  echo "------------------------------------------------------------"

  if [[ -s "$PARENTS" && -s "$PARENT_SUMMARY" ]]; then
    echo "[V4C:G${GEN_PAD}] Parent selection already complete; resuming."
  else
    echo "[V4C:G${GEN_PAD}] Selecting frozen-bottleneck parents"
  ...
  if [[ -s "$CANDIDATES" && -s "$PROPOSALS" && -s "$GENERATION_SUMMARY" ]]; then
    echo "[V4C:G${GEN_PAD}] Offspring generation already complete; resuming."
  else
    echo "[V4C:G${GEN_PAD}] Generating up to $N_OFFSPRING V4C offspring"
    python v4c/03_generate_offspring.py \
      --generation "$GEN" \
      --parents "$PARENTS" \
  ...
  if [[ -s "$APEX_SCORED" ]]; then
    echo "[V4C:G${GEN_PAD}] APEX scores already exist; resuming."
  else
    if [[ -z "$APEX_SCORER_CMD" ]]; then
      cat >&2 <<EOF
[V4C:G${GEN_PAD}] APEX_SCORER_CMD is required because no score file exists.
Set a command template containing:
  {input}       candidate CSV
...
EOF
    ...
    echo "[V4C:G${GEN_PAD}] Running APEX scorer"
    echo "[V4C:G${GEN_PAD}] $APEX_COMMAND"
    eval "$APEX_COMMAND"

    if [[ ! -s "$APEX_SCORED" ]]; then
      echo "[V4C:G${GEN_PAD}] APEX output is missing or empty: $APEX_SCORED" >&2
      exit 5
    fi
  fi

  if [[ -s "$SURVIVORS" && -s "$SCORED_OFFSPRING" && -s "$SURVIVOR_SUMMARY" ]]; then
    echo "[V4C:G${GEN_PAD}] Survivor selection already complete; resuming."
  else
    echo "[V4C:G${GEN_PAD}] Selecting $N_SURVIVORS fitness-diverse survivors"
  ...
  if [[ -s "$LATENT_VECTORS" && -s "$LATENT_METADATA" && -s "$LATENT_SUMMARY" ]]; then
    echo "[V4C:G${GEN_PAD}] Survivor encoding already complete; resuming."
  else
    echo "[V4C:G${GEN_PAD}] Re-encoding survivor population"
...
print(f"[V4C:G{generation:02d}] Integrity check passed for {len(survivors):,} survivors")
PY
...
requested_offspring = int(sys.argv[3])
generations = []
...
manifest = {
    ...
    "requested_offspring_per_generation": requested_offspring,
    "maximum_requested_total": requested_offspring * 10,
    "completed_generations": [item["generation"] for item in generations],
    "accepted_offspring_total": sum(
        int(item["generation_summary"].get("accepted_offspring", 0))
        for item in generations
    ),
    "generations": generations,
}

path = root / "v4c_evolution_manifest.json"
path.write_text(json.dumps(manifest, indent=2, default=str), encoding="utf-8")
print(f"[V4C] Evolution manifest written for {len(generations)} completed generations")
print(f"[V4C] Manifest: {path}")
PY

echo "============================================================"
echo "[V4C] Requested generation range complete"
echo "[V4C] Manifest: $RESULTS_ROOT/v4c_evolution_manifest.json"
echo "============================================================"
